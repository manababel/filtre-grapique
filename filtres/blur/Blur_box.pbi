...
Procedure BoxBlur_X(*FilterCtx.FilterParams) 
  ...
  For y = thread_start To thread_stop -1
    ...
    *dst\pixel[lg * y] = (((a * blur)+ 32768) & $FF0000) << 8 | (((r * blur)+ 32768) & $FF0000) | (((g * blur)+ 32768) & $FF0000) >> 8 | (((b * blur)+ 32768) >> 16)
    For x = 0 To lg - 2
      pixel_out = *src\pixel[*pointeur\l[x ]]
      pixel_in  = *src\pixel[*pointeur\l[x + optx]]
      a + ( pixel_in >> 24       ) -  (pixel_out >> 24)
      r + ((pixel_in >> 16) & $FF) - ((pixel_out >> 16) & $FF)
      g + ((pixel_in >>  8) & $FF) - ((pixel_out >>  8) & $FF)
      b + ( pixel_in        & $FF) -  (pixel_out        & $FF)
      *dst\pixel[lg * y + x + 1] = ((a * blur) & $FF0000) << 8 | ((r * blur) & $FF0000) | ((g * blur) & $FF0000) >> 8 | ((b * blur) >> 16)
    Next
  Next
EndProcedure
...
Procedure BoxBlur_Y(*FilterCtx.FilterParams) 
  ...
  Protected.l idx_x
  Protected.l offset_out
  Protected.l offset_in
  ...
  For y = 0 To ht - 2
    offset_out = *pointeur\l[y] * lg
    offset_in  = *pointeur\l[y + opty] * lg
    *src = *FilterCtx\addr[0] ; Pointeur de base
    For x = thread_start To thread_stop - 1
      ...
      *dst\pixel[((y + 1 )* lg) + x] = ((arr_a(idx_x) * blur) & $FF0000) << 8 | ((arr_r(idx_x) * blur) & $FF0000) | ((arr_g(idx_x) * blur) & $FF0000) >> 8 | ((arr_b(idx_x) * blur) >> 16)
    Next
  Next
EndProcedure
  ...
  With FilterCtx
    ...
    If \addr[4] And \addr[2]  And \addr[3] 
      ...
      For boucle = 0 To \option[2] - 1
        If boucle = 0
          Blur_box_call(BoxBlur_X, x , \image[0] ,\addr[4])
        Else
          Blur_box_call(BoxBlur_X, x , \image[1] ,\addr[4])
        EndIf ; passe x
