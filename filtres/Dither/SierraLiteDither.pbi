...
Procedure SierraLiteDither_MT(*FilterCtx.FilterParams)
  With *FilterCtx
...
Procedure SierraLiteDitherEx(*FilterCtx.FilterParams)
  Restore SierraLiteDither_data
  Protected last_data = Filter_InitAndValidate()
  If last_data < 0 : ProcedureReturn 0 : EndIf
  
  With *FilterCtx
    ; Lancement du filtre (1 thread pour l'ordre séquentiel de la diffusion d'erreur)
    Create_MultiThread_MT(@SierraLiteDither_MT(), 1)
    
    mask_update(*FilterCtx.FilterParams, last_data)
  EndWith
EndProcedure

Procedure SierraLiteDither(source, cible, mask, levels, gray)
  Set_Source(source)
  Set_Cible(cible)
  Set_Mask(mask)
  With FilterCtx
    ...
    \option[1] = gray
  EndWith
  SierraLiteDitherEx(FilterCtx.FilterParams)
EndProcedure

DataSection
  SierraLiteDither_data:
  Data.s "SierraLiteDither"
  Data.s "Sierra Lite (version rapide)"
